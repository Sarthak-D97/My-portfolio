...
import { ParallaxFigure } from "@/components/ui/parallax-figure";
import { RunningHead } from "@/components/ui/section";
    ...
    <div className="border-y border-border">
      ...
      >
        <Container className="relative">
          {/* Running head — same construction as every other section, plus the brand mark. */}
          <div className="flex flex-wrap items-baseline justify-between gap-4">
            <RunningHead index="04" label="Case study — Founder" />
            <span className="flex items-center gap-3">
              <Image src={lireons.logo} alt="" width={28} height={28} className="size-7 object-contain" />
              <ExternalLink href={lireons.url} className="meta link-ink text-fg">
                {lireons.domain}
              </ExternalLink>
            </span>
          </div>
          ...
          <div className="grid gap-x-6 lg:grid-cols-12">
            <h2 id="lireons-title" data-reveal="clip" className="mt-8 text-display-lg text-fg sm:mt-10 lg:col-span-9">
              {lireons.title.join(" ")}
            </h2>
            <p className="mt-5 max-w-[60ch] text-lede text-muted lg:col-span-8">{lireons.subtitle}</p>
          </div>

          {/* Body grid: spec sheet (sticky) + chapters */}
          <div className="mt-12 grid gap-x-6 gap-y-12 lg:mt-16 lg:grid-cols-12">
